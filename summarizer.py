...
def render_output_dashboard(model_used=None):
    if st.session_state.last_summary:
        # Replicated exactly from the design footprint layout in image_1f53e3.png
        st.markdown(f"""
        ...
        # Micro Action Layout Row Setup 
        col_fav, col_copy = st.columns([1, 4])
        ...
        with col_copy:
            # Native Streamlit click-to-copy code layout wrapper window engine
...
def render_text_workspace(api_key):
    ...
    raw_text = st.text_area("Source Text Dropzone Block:", key="text_input_box", height=250, placeholder="Paste your transcripts passages directly into this block area...")
    min_limit, max_limit = st.slider("Target Length Footprint (Words count limits):", 40, 300, (50, 120))
    st.markdown("<br>", unsafe_allow_html=True)
